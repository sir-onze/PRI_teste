...
router.get('/get', function(req, res, next) {
    var id = req.query.id;
    if(id!=null){
    axios.get('http://clav-api.dglab.gov.pt/api/tipologias/tip_'+id+'?info=completa&apikey=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE1Nzg4NjAwNTQsImV4cCI6MTU4MTQ1MjA1NH0.HIlH4_Ao6504qaLhhbZ2_OtDzaZaG5FeYy-Yc2d9lwQ')
    .then(dados => {
        res.render('id_page', { lista: dados.data });
    })
    .catch(erro => {
        res.render('error', {error: erro})
    })
...
router.get('/get/entidade/:id', function(req, res, next) {
    ...
    .then(dados => {
        res.render('sigla_page', { lista: dados.data });
    })
    .catch(erro => {
        res.render('error', {error: erro})
    })
